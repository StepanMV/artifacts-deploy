...
CoolerList::CoolerList(const QString &name, const QList<QPushButton *> &buttons,
                       QListWidget *listView, CoolerDialog *dialog,
                       QObject *parent)
    : QObject{parent} {
  this->name = name;
  this->list = listView;
  ...
  connect(buttons.at(1), &QPushButton::clicked, this,
          &CoolerList::onRemoveClick);
  connect(buttons.at(2), &QPushButton::clicked, this,
...
void CoolerList::onDialogAccept() {
  ...
  dialog->clearFileds();
  if (data.value("display").toString().isEmpty())
    return;
  if (editing == -1) {
